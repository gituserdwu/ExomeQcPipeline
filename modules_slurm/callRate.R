...
dat <- read.table("postcalling_qc/callRate_bychr.txt", header=TRUE, sep="\t")
#colnames(dat)=c("CHROM","MISSING_RATE")

dat2 <- read.table("postcalling_qc/GT.txt", header=TRUE, sep="\t")


jpeg(filename = "postcalling_qc/callRate_bychr_scatter.jpeg", width = 1000, height = 480, units = "px",bg ="white")
data_cum <- dat %>%
   group_by(CHROM) %>%
...
manhplot <- ggplot(plot_data, aes(x = bp_cum, y = MISSING_RATE, color = as_factor(CHROM))) +
 geom_point(alpha = 0.75) +
 scale_x_continuous(label = axis_set$CHROM, breaks = axis_set$center) +
 scale_y_continuous(expand = c(0,0), limits = c(0, 1)) +
 scale_color_manual(values = rep(c("#276FBF", "#183059"), unique(length(axis_set$CHROM)))) +
 scale_size_continuous(range = c(0.5,3)) +
 labs(x = NULL, y="missing call rate") +
 theme_minimal() +
 theme(legend.position = "none",panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank(),axis.text.x = element_text(angle = 60, size = 8, vjust = 0.5))
print(manhplot)
dev.off()
